...
class DataManager:
    ...
    """
    __attribute_value_mapping: dict[AttributeUID, set[AttributeValueUID]]
    __value_attribute_mapping: dict[AttributeValueUID, AttributeUID]
    __participant_mapping: dict[
        ParticipantUID, tuple[set[AttributeUID], set[AttributeValueUID]]
    ]
    """
    ...
    def __map_value_name(
        ...
        self.__uid_name_mapping[uid] = name
        self.__name_uid_mapping[(name, attribute_uid)] = uid
    ...
    def __init__(self, data: list[list[str]]) -> None:
        ...
        """
        self.__data_uids = [[AttributeValueUID()] * len(data[0])] * (len(data) - 1) + [
            [AttributeUID()] * len(data[-1])
        ...
        current_uid: UID = 0
        for i in range(len(data) - 1):
            self.__data_uid_offsets.insert(0, current_uid)
            ...
            for j in range(len(data[i])):
                if (
                    ...
                    current_uid += 1
                self.__data_uids[i][j] = self.__name_uid_mapping[data[i][j]]
